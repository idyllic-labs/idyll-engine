...
const commands = {
  ...
  '/load': async (args: string[]) => {
    ...
    try {
      ...
      if (!('type' in parsed) || parsed.type !== 'agent') {
        console.log(chalk.red('Error: File does not contain an agent document'));
        return;
      }
      ...
      if (!modelCheck.valid) {
        ...
        console.log(chalk.gray('   You can still load the agent but chat will fail without API keys.'));
      }
      ...
      console.log(chalk.green(`✅ Loaded agent: ${agentDoc.name || 'Unnamed Agent'}`));
  ...
  '/memory': () => {
    ...
    activities.forEach((activity, i) => {
      ...
      if (activity.userMessage) {
        console.log(`  User: ${activity.userMessage.substring(0, 80)}${activity.userMessage.length > 80 ? '...' : ''}`);
      }
      if (activity.assistantMessage) {
        console.log(`  Assistant: ${activity.assistantMessage.substring(0, 80)}${activity.assistantMessage.length > 80 ? '...' : ''}`);
      }
      if (activity.toolCalls) {
        console.log(`  Tools: ${activity.toolCalls.map(tc => tc.name).join(', ')}`);
      }
      if (activity.error) {
  ...
  '/tools': () => {
    ...
    Object.entries(tools).forEach(([name, tool]) => {
      console.log(`  ${chalk.green(name)} - ${tool.description || 'No description'}`);
    });
  },
...
async function chat(input: string) {
  if (!currentAgent) {
    console.log(chalk.red('No agent loaded. Use /load <path> to load an agent.'));
    return;
  }
...
async function main() {
  ...
  console.log(chalk.gray('Type /help for commands or /load <path> to get started\n'));
